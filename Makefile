...
import-viewer-subset:
	...
	python3 viewer/import_archive_subset.py "$(SOURCE)" --limit $(LIMIT) --strategy $(MODE) $(if $(filter 1,$(REQUIRE_MEDIA)),--require-media,) $(if $(filter 1,$(REQUIRE_LOCAL_MEDIA)),--require-local-media,)
